...
// Algorithm
// We use divide and conqour to divide the array to individuals as left and right subarrays and merge.
// Now after the division of arrays, we merge the array and count the inversions as c=c+(m-i).
...
{ 
	...
	while ((i <= mid - 1) && (j <= right)) { 
		if (arr[i] <= arr[j]) { 
			...
			temp[k++] = arr[j++]; 			
			inv_count = inv_count + (mid - i); //count the inversions as mid -i elements are 
		} 
	} 
	...
	/*Copy back the merged sorted elements to original array*/
